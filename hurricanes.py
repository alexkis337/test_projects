...
for elem in damages:
    if elem.find('M') >= 0:
        damages[damages.index(elem)] = float(elem.replace('M', '')) * 1000000
    elif elem.find('B') >= 0:
        damages[damages.index(elem)] = float(elem.replace('B', '')) * 1000000000
    else:
        damages[damages.index(elem)] = 'Damages not recorded'

# creating dictionary with key=Name
dict_names = {}
for index in range(0, 34):
    dict_names[names[index]] = {"Name": names[index],
                                "Month": months[index],
                                "Year": years[index],
...
# creating a function to select all the hurricanes for a given year
def hurricane_by_years(year):
    dict_years = {}
    for value in dict_names:
        if dict_names[value]['Year'] not in dict_years.keys():
            dict_years[dict_names[value]['Year']] = dict_names[value]
        else:
            dict_years[dict_names[value]['Year']].append(dict_names[value])
    print(dict_years[year])
    return(dict_years)

print('<<<<')
hurricane_by_years(1932)
print('<<<<')
...
for elem in locations:
    if elem not in dict_locations.keys():
        dict_locations[elem] = 1
    else:
        dict_locations[elem] += 1

most = 0
for elem in dict_locations:
    if dict_locations[elem] > most:
        most = dict_locations[elem]
        country = elem

print(dict_locations)
print(f'Most fucked location is {country}')
